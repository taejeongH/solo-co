...
생성형 AI 기반 개인·그룹 맞춤 여행 일정 생성 및 협업 계획 플랫폼
...
* 핵심 개발 기간: 2025.11 ~ 2025.12 (5주)
* 후속 리팩토링 및 기능 개선: 2026.02

### 배경

* 1인 특화 장소 추천과 그룹 여행 일정 협업을 하나의 서비스 흐름 안에서 제공하는 플랫폼이 필요했습니다.
* 특히 그룹 여행은 일정 변경 공유와 여행지 후보 결정이 여러 채널로 분산되어 있어 협업 비효율이 컸습니다.

### 목표

* AI 기반 코스 후보 추천과 동선 자동 생성 기능을 구현합니다.
* 그룹 여행에서 일정 변경, 채팅, 투표를 실시간으로 공유할 수 있는 협업 구조를 제공합니다.
* 외부 API 연동과 실시간 이벤트 전파가 함께 존재하는 구조에서 기본적인 응답성과 일관성을 확보합니다.

### 서비스 개요

* 1인 특화 여행 장소를 AI로 추천받고, 그룹 멤버가 WebSocket을 통해 일정 변경 사항을 실시간으로 공유할 수 있는 여행 일정 생성 및 협업 플랫폼입니다.

### 역할

* 백엔드 전반 설계 및 구현
* JWT 인증, 여행 프로젝트 관리, AI 일정 생성, Google Places API 연동
...
| 기능 | 설명 |
| ---- | ---- |
| LLM 기반 여행 코스 후보 생성 | 여행 기간과 선택 장소 정보를 기반으로 개인·그룹 여행 코스 후보 생성 |
| 여행 동선 자동 생성 | `Nearest Neighbor` 알고리즘으로 기본 동선을 자동 구성 |
| WebSocket 기반 실시간 협업 | 일정 변경 이벤트와 채팅 메시지를 프로젝트 단위 채널로 브로드캐스트 |
| 그룹 커뮤니티 및 투표 | 게시글, 댓글, 투표 기능을 통해 그룹 의사결정 지원 |
...
* 여행 기간과 선택 장소 정보를 기반으로 개인·그룹 여행 코스 후보를 생성하고, 선택된 장소는 `Nearest Neighbor` 알고리즘으로 정렬해 기본 동선을 자동 구성했습니다.
* 경로 계산까지 전부 LLM에 맡기면 응답 시간이 길고 비용이 커질 수 있어, 경로 계산은 알고리즘으로 처리하고 AI는 코스 후보 생성 역할에 한정했습니다.
* 이를 통해 기본 동선 생성 속도를 확보하면서도 추천 경험을 함께 제공할 수 있도록 설계했습니다.

### 5-2. 외부 장소 API 비동기 호출 구조

* 장소 검색, 상세 조회, 사진 조회는 Google Places API를 사용하며 `WebClient` 기반 비동기 호출로 처리합니다.
* 외부 API 지연이 전체 요청 흐름의 병목이 되지 않도록 비동기 호출 결과를 후속 처리와 연결하는 구조로 기존 동기 호출 흐름을 리팩토링했습니다.
* 일정 생성 과정에서 AI가 새 장소를 제안한 경우, 해당 장소를 다시 Google Places API로 조회해 실제 장소 정보로 보강합니다.

### 5-3. 실시간 협업 및 데이터 정합성 처리

* 일정 변경 이벤트와 채팅 메시지는 WebSocket(STOMP) 채널을 통해 프로젝트 단위로 브로드캐스트합니다.
* 일정 추가·수정·삭제 상태를 참여자에게 즉시 공유할 수 있도록 구성하고, 일정 관련 REST API에는 프로젝트 멤버 권한 검증을 적용했습니다.
...
* 애플리케이션의 조회 기반 검증만으로는 동시 요청을 완전히 막기 어렵다고 판단해 DB 제약을 최종 방어선으로 두었습니다.
...
* 비동기 호출 결과를 후속 처리와 연결하는 방식으로 기존 동기 호출 흐름을 리팩토링했습니다.
* 비동기 처리 전환 과정에서 발생한 인증 오류를 해결하기 위해 JWT 필터 설정을 개선했습니다.
